...
export function sanitizeProfileData(data) {
  ...
  s.category = sanitizeString(s.category || 'general', 60);
  if (Array.isArray(s.keywords)) {
    s.keywords = s.keywords.slice(0, 10).map((k) => sanitizeString(k, 50)).filter(Boolean);
  }
  ...
  if (s.theme) {
    s.theme = {
      ...
      template: sanitizeString(s.theme.template, 50) || 'starter',
    };
  }
