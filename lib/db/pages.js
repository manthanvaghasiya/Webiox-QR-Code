// ─────────────────────────────────────────────────────────────────────────────
// HOSTED PAGES DB LAYER
//
// Backs `app/p/[id]/page.js` — public landing pages for QR types like
// social/pdf/event/gallery/etc. Stored in the `pages` collection.
// Each page has a public token (in URL), an edit token (for /edit/{token}),
// and an arbitrary `body` JSON shaped per page type.
// Exports CRUD helpers + token generation.
// ─────────────────────────────────────────────────────────────────────────────
...
/**
 * Convert a raw MongoDB document into a plain, serializable JS object.
 * - ObjectId  → string
 * - Date       → ISO string
 * Prevents the "Objects with toJSON methods are not supported" error when
 * passing the result as props to a Client Component.
 */
function serializePage(doc) {
  if (!doc) return null;
  return JSON.parse(JSON.stringify(doc, (_, v) => {
    // BSON ObjectId / any object with a toHexString method
    if (v != null && typeof v === 'object' && typeof v.toHexString === 'function') {
...
export async function getPageByEditToken(editToken) {
  const col = await pagesCol();
  const doc = await col.findOne({ editToken });
  return serializePage(doc);
}
