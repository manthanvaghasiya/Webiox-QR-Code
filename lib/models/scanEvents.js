// ─────────────────────────────────────────────────────────────────────────────
// SCAN EVENTS MODEL — Per-scan analytics records
//
// Collection: `scan_events`  (TTL: 1 year, auto-purged by MongoDB)
// Fields: type, qrCodeId, profileId, timestamp, ip (SHA-256 hashed for privacy),
//   country, region, city, device, os, browser, referrer
// One row per scan/visit. Aggregated for dashboard charts and AI insights.
// ─────────────────────────────────────────────────────────────────────────────
...
export async function aggregateBy(db, qrCodeId, field, { start, end } = {}) {
  const match = { qrCodeId: new ObjectId(qrCodeId), [field]: { $ne: null } };
  if (start || end) {
    match.timestamp = {};
  ...
  return db.collection(COLLECTION).aggregate([
    { $match: match },
    { $group: { _id: `$${field}`, count: { $sum: 1 }, unique: { $addToSet: '$ip' } } },
...
export async function uniqueScanCount(db, qrCodeId, { start, end } = {}) {
  const match = { qrCodeId: new ObjectId(qrCodeId), ip: { $ne: null } };
  if (start || end) {
    match.timestamp = {};
    if (start) match.timestamp.$gte = new Date(start);
    if (end) match.timestamp.$lte = new Date(end);
...
export async function totalScanCount(db, qrCodeId, { start, end } = {}) {
  const filter = { qrCodeId: new ObjectId(qrCodeId) };
  if (start || end) {
    filter.timestamp = {};
    if (start) filter.timestamp.$gte = new Date(start);
