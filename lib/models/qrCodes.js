// ─────────────────────────────────────────────────────────────────────────────
// QR CODES MODEL — Every QR a user creates
//
// Collection: `qr_codes`
// Fields: userId, type, shortId (for dynamic /r/{shortId}), destination,
//   staticContent, design (colors/pattern/logo), isDynamic, isPaused,
//   campaign dates, folderId, scanCount
// Dynamic QRs encode /r/{shortId} → app/r/[shortId]/route.js redirects.
// Static QRs encode the content directly (vCard, URL, WiFi, etc.).
// ─────────────────────────────────────────────────────────────────────────────
...
export async function createQrCode(db, data) {
  ...
  const doc = {
    ...
    config: data.config ?? {}, // Type-specific configuration (WiFi SSID/password, vCard fields, etc.)
    design: {
...
export async function findQrCodesByUser(
  db,
  userId,
  { folderId, status, search, sort = 'recent', limit = 100, skip = 0, type } = {}
) {
