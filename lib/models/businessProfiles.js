// ─────────────────────────────────────────────────────────────────────────────
// BUSINESS PROFILES MODEL — Mini-website for a business
//
// Collection: `business_profiles`
// Fields: slug (unique URL part), userId, qrCodeId, biolinkId, businessName,
//   category, keywords, tagline, logoUrl, coverImageUrl, about, contact,
//   address (with lat/lng), businessHours, socialLinks, services, gallery,
//   reviews, theme, totalScans/Calls/Directions/ContactSaves
// Public URL: /b/{slug}  ·  Auto-creates a QR code + biolink on insert.
// ─────────────────────────────────────────────────────────────────────────────
...
// Returns candidate slug, but doesn't persist anything
export function generateSlugCandidate(businessName, forceRandom = false) {
  const base = slugify(businessName || 'business');

  // Clean slug if not reserved
  if (!forceRandom && base && !RESERVED_SLUGS.has(base)) {
    return base;
  }

  // Return slug with random suffix
  return `${base || 'biz'}-${nanoid6()}`;
}

export async function generateSlug(db, businessName) {
  const col = db.collection(COLLECTION);

  // Try 5 times to get a unique slug
  for (let attempt = 0; attempt < 5; attempt++) {
    const forceRandom = attempt > 0;
    const candidate = generateSlugCandidate(businessName, forceRandom);
...
export async function createProfile(db, data) {
  ...
  const doc = {
    ...
    biolinkId: data.biolinkId ? toObjectId(data.biolinkId) : null,
    biolinkSlug: data.biolinkSlug || null,
    slug,
    ...
    theme: {
      ...
      template: data.theme?.template || 'starter',
    },
...
const ALLOWED_UPDATE_FIELDS = [
  'businessName', 'category', 'keywords', 'tagline', 'logoUrl', 'coverImageUrl', 'about',
