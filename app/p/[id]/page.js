...
export default async function DynamicPage({ params }) {
  ...
  try {
    ...
    const db = client.db();
    page = await db.collection('dynamic_pages').findOne({ shortId: id });
  } catch {
    notFound();
  }
    ...
    <div className="relative min-h-screen bg-gradient-to-br from-slate-900 via-purple-950 to-slate-900 flex flex-col items-center py-16 px-4 overflow-hidden">
      ...
      <div className="relative z-10 w-full max-w-sm mx-auto">
        ...
        <div className="flex flex-col items-center mb-10 text-center">
          ...
          {page.description && (
            <p className="mt-2 text-sm text-gray-400 max-w-xs leading-relaxed">{page.description}</p>
          )}
        </div>
          ...
          {page.links.map((link, i) => {
            ...
            return (
              ...
              >
                ...
                <span className="flex-1 text-left text-[15px]">{link.platform}</span>
